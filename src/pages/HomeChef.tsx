...
const HomeChef = () => {
  ...
  const [pedidosPendientesDeCocina, setPedidosPendientesDeCocina] = useState<
    any[]
  >([]);
  ...
  useEffect(() => {
    traer("pedidos", "", (data) => {
      ...
      setPedidosPendientesDeCocina(pedidosPendientesCocina); // Actualiza el estado una sola vez
    });
  }, []);

  const requiereCocina = (pedido: any): boolean => {
    return pedido.contenidoDelPedido.some(
      (item: any) => item.tipo === "comida"
    );
  };
  ...
  const marcarPedidoComoListo = (pedido: any, estadoAAsignar: string) => {
    pedido.comidaLista = true;
    if (pedido.bebidaLista) {
    ...
    modificar("pedidos", pedido).then(() => {
      if (pedido.comidaLista && pedido.bebidaLista) {
        ...
        Swal.fire({
          title: "Listo",
          text: "Pedido marcado como comida lista",
          icon: "success",
          heightAuto: false,
          timer: 3000,
          confirmButtonText: "Cerrar",
